...
sub setup_canvas {
  my $root = %widgets<canvas>.get_root_item;

  %shapes<ellipse1> = Goo::Ellipse.new($root, 0, 0, 25, 15);
  with %shapes<ellipse1> {
    .translate(100, 100);
    .animation-finished.tap(-> *@a { say 'Animation finished' });
  }
  %shapes<rect1> = Goo::Rect.new($root, -10, -10, 20, 20);
    %shapes<rect1>.translate(100, 200);
  %shapes<rect3> = Goo::Rect.new($root, -10, -10, 20, 20);
    %shapes<rect3>.translate(200, 200);

  %shapes<ellipse2> = Goo::Ellipse.new($root, 0, 0, 25, 15);
  %shapes<ellipse2>.translate(100, 400);
  %shapes<rect2> = Goo::Rect.new($root, -10, -10, 20, 20);
  %shapes<rect2>.translate(100, 500);
  %shapes<rect4> = Goo::Rect.new($root, -10, -10, 20, 20);
