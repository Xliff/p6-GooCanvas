...
sub setup-canvas ($canvas) {
  ...
  set-data($_, 'id', "Line #{ $count++ }") for (
    $path1 = Goo::Path.new($root, 'M 20 20 L 40 40'),
             Goo::Path.new($root, 'M30 20 l20, 20'),
             Goo::Path.new($root, 'M 60 20 H 80'),
             Goo::Path.new($root, 'M60 40 h20'),
             Goo::Path.new($root, 'M 100,20 V 40'),
             Goo::Path.new($root, 'M 120 20 v 20'),
             Goo::Path.new($root, 'M 140 20 h20 v20 h-20 z')
  );
  $path1.line-width = 4;

  with Goo::Path.new($root, 'M 180 20 h20 v20 h-20 z m 5,5 h10 v10 h-10 z') {
    .fill-color = 'red'; .fill-rule = FILL_RULE_EVEN_ODD;
    set-data($_, 'id', "Line #{ $count++ }");
  }

  with Goo::Path.new($root, 'M 220 20 L 260 20 L 240 40 z') {
  ...
  set-data($_, 'id', "Curve #{ $count++ }") for (
    Goo::Path.new($root, 'M20,100 C20,50 100,50 100,100 S180,150 180,100'),
    Goo::Path.new($root, 'M220,100 c0,-50 80,-50 80,0 s80,50 80,0'),
	  Goo::Path.new($root, 'M20,200 Q60,130 100,200 T180,200'),
    Goo::Path.new($root, 'M220,200 q40,-70 80,0 t80,0')
  );

  # Test the elliptical arc commands: Aa.
  $count = 0;
  with Goo::Path.new($root, 'M200,500 h-150 a150,150 0 1,0 150,-150 z') {
    .fill-color = 'red';    .stroke-color = 'blue'; .line-width = 5;
    set-data($_, 'id', "Arc #{ $count++ }")
  }
  with Goo::Path.new($root, 'M175,475 v-150 a150,150 0 0,0 -150,150 z') {
    .fill-color = 'yellow'; .stroke-color = 'blue'; .line-width = 5;
    set-data($_, 'id', "Arc #{ $count++ }")
  }
  with Goo::Path.new($root, q:to/PATH/)
M400,600 l 50,-25
...
PATH
  ...
  with Goo::Path.new($root, 'M 525,75 a100,50 0 0,0 100,50') {
    .stroke-color = 'red'; .line-width = 5;
    set-data($_, 'id', "Arc #{ $count++ }")
  }
  with Goo::Path.new($root, 'M 725,75 a100,50 0 0,1 100,50') {
    .stroke-color = 'red'; .line-width = 5;
    set-data($_, 'id', "Arc #{ $count++ }")
  }
  with Goo::Path.new($root, 'M 525,200 a100,50 0 1,0 100,50') {
    .stroke-color = 'red'; .line-width = 5;
    set-data($_, 'id', "Arc #{ $count++ }")
  }
  with Goo::Path.new($root, 'M 725,200 a100,50 0 1,1 100,50') {
    .stroke-color = 'red'; .line-width = 5;
    set-data($_, 'id', "Arc #{ $count++ }")
  }
}
